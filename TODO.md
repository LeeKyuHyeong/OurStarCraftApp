...
초패스트 다크 vs 5팩 물량
전체를 뒤짚어야겠는데..
1. 테란이 배럭더블이라 배럭-마린-앞마당 건설까지 너무 완벽한데 여기에 대응하는 프로토스가 '확장 서두르네요' 가 나오면 초패스트 다크가 아니잖아요?
2. 시타델 오브 아둔 이 건물이름인데 시타델보다는 아둔으로 많이 얘기되죠 수정해주세요
3. '지금은 운영의 시간입니다' 는 둘의 경기에 어울리지 않아요
4. 초패스트 다크는 드랍보다는 육지를 통해 뚫는 거에요 당연하게도 셔틀을 뽑을 로보틱스 자원+그럴시간 도 없기때문이에요 그러니까 멘트 바꿔주세요
5. 사실상 감지도구(아카데미 이후 스캔, 엔지니어링베이 이후 터렛, 벌쳐 마인업 이후 마인)이 없는 순간 테란은 다크를 방어하기 어려워요
6. 프테전 중반 쯤에 나오는 '질럿 돌진! 마린 라인 정면 돌파!' 는 굉장히 어색하네요 질럿+드라군 vs 탱크+벌쳐가 프테전의 99% 병력 구성이에요
7. 여기서 나오는 '피드백! 상대 에너지 유닛 폭발!' 은 다크아콘의 스킬을 말하는 것 같아요 이게 성공하려면 선행되는게 프로토스쪽의 다크아콘 합체 + 테란쪽의 에너지 유닛 생산(보통 베슬이겠죠) 이 확인되고 로그에 나올 수 있으면 좋겠어요
8. 프로토스선수의 로그에서 '드랍 루트를 미리 읽었습니다! 탱크가 대기하고 있었는데요!' 는 테란쪽 로그같아요 확인하고 수정해주세요 반대 입장에서 나와야하는거라면 '리콜 루트를 미리 읽었습니다' 가 맞을 것 같고 프로토스 입장이면 드라군이 대기하고있는게 맞겠어요
9. 프테전 후반 쯤 나오는 테란의 '마린 스플릿! 스톰 피해 최소화!' 는 어색해요 위에서 말했듯이 테란의 병력은 대부분 탱크+벌쳐+골리앗(메카닉)일거에요
10.
...
현재 저표본(n=25~50/dir)에서 >15%p 편향이 관측된 시나리오들을 n=150/dir로 재검증하여 진짜 문제만 수정.

### 대상 시나리오

| 시나리오 | 현재 편향 | 현재 n/dir | 우선순위 | 비고 |
|---------|----------|-----------|---------|------|
| PvT S9 (mine_triple) | 28%p | 25 | 높음 | 이전 사이클 4%p → 28%p 변동, 노이즈 가능성 |
| ZvZ S8 (12풀 vs 3해처리) | 22%p | 50 | 중간 | 이전 사이클 3%p → 22%p 변동 |
| ZvZ S5 (4풀 vs 9풀) | 20%p | 50 | 중간 | 이전 사이클 8%p → 20%p 변동 |
| TvT S12 (wraith_mirror) | 26%p | 30 | 중간 | 미러인데 편향 큼 |

### 진행 방법

```
① n=150/dir 고표본 테스트 실행
   → 편향 ≤10%p → 노이즈 확인, 수정 불필요
   → 편향 >10%p → 구조적 문제, 수정 필요

② 원인 분석 (3가지 체크)
   - baseProbability 비대칭?
   - army 피해 비대칭? (결전 직전 Phase의 누적 army 추적)
   - 분기 수 불균형? (home승 2개 vs away승 1개 등)

③ 소폭 조정 (한번에 1~2개 값만)
   - army: ±1~2 단위
   - baseProbability: ±0.1 단위
   - 대폭 수정은 과잉 보정 유발
