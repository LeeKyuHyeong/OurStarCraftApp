...
/// PvP 전 시나리오 10게임 전체 로그 출력 테스트
@Timeout(Duration(minutes: 30))
void main() {
  TestWidgetsFlutterBinding.ensureInitialized();

  final scenarios = <_Scenario>[
    _Scenario(1, '드라군 넥서스 미러', 'pvp_2gate_dragoon', 'pvp_2gate_dragoon'),
    _Scenario(2, '드라군 vs 노겟넥서스', 'pvp_2gate_dragoon', 'pvp_1gate_multi'),
    _Scenario(3, '로보리버 vs 투겟드라군', 'pvp_1gate_robo', 'pvp_2gate_dragoon'),
    _Scenario(4, '다크 올인 vs 드라군', 'pvp_dark_allin', 'pvp_2gate_dragoon'),
    _Scenario(5, '질럿러시 vs 스탠다드', 'pvp_zealot_rush', 'pvp_2gate_dragoon'),
    _Scenario(6, '다크 vs 질럿러시', 'pvp_dark_allin', 'pvp_zealot_rush'),
    _Scenario(7, '로보틱스 미러', 'pvp_1gate_robo', 'pvp_1gate_robo'),
    _Scenario(8, '4게이트 vs 원겟멀티', 'pvp_4gate_dragoon', 'pvp_1gate_multi'),
    _Scenario(9, '질럿러시 vs 로보리버', 'pvp_zealot_rush', 'pvp_1gate_robo'),
    _Scenario(10, '다크 미러', 'pvp_dark_allin', 'pvp_dark_allin'),
  ];

  for (final sc in scenarios) {
    group('PvP S${sc.num}: ${sc.title}', () {
      ...
      tearDownAll(() {
        ...
        buf.writeln('# PvP S${sc.num}: ${sc.title}');
        ...
        File('test/output/pvp_s${sc.num}_10games.md')
