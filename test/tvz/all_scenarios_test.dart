/// TvZ 전체 49개 시나리오 3경기 로그 + 1000경기 통계
import 'dart:io';
...
void main() {
  ...
  // 7 오프닝 빌드
  const terranBuilds = [
    ('tvz_nobar_double', 'nobar_double'),
    ('tvz_bar_double', 'bar_double'),
    ('tvz_111', '111'),
    ('tvz_2bar_academy', '2bar_academy'),
    ('tvz_fac_double', 'fac_double'),
    ('tvz_2star', '2star'),
    ('tvz_bbs', 'bbs'),
  ];
  ...
  // 7 저그 빌드
  const zergBuilds = [
    ('zvt_trans_2hatch_mutal', '2hatch_mutal'),
    ('zvt_4pool', '4pool'),
    ('zvt_trans_530_mutal', '530_mutal'),
    ('zvt_trans_lurker_defiler', 'lurker_defiler'),
    ('zvt_trans_mutal_lurker', 'mutal_lurker'),
    ('zvt_trans_mutal_ultra', 'mutal_ultra'),
    ('zvt_trans_ultra_hive', 'ultra_hive'),
  ];

  // 49개 시나리오 생성
  final scenarios = <Map<String, String>>[];
  for (final (tId, tLabel) in terranBuilds) {
    for (final (zId, zLabel) in zergBuilds) {
      scenarios.add({'home': tId, 'away': zId, 'label': '${tLabel}_vs_$zLabel'});
    }
  }
  ...
  Future<void> run3Games(String homeBuild, String awayBuild, String label) async {
    ...
    buf.writeln('# TvZ $label 3경기 로그');
    buf.writeln();
    buf.writeln('**빌드**: $homeBuild vs $awayBuild');
    buf.writeln('**선수**: ${homePlayer.name}(테란/홈) vs ${awayPlayer.name}(저그/어웨이) | 동일 능력치 700');
    buf.writeln();
    ...
    for (int game = 1; game <= 3; game++) {
      ...
      for (int i = 0; i < state.battleLogEntries.length; i++) {
        ...
        final ownerTag = e.owner == LogOwner.home ? '[T]  ' : e.owner == LogOwner.away ? '[Z]  ' : '[해설]';
    ...
    buf.writeln('---');
    buf.writeln('## 종합: T $homeWins승 / Z ${3 - homeWins}승');

    final outDir = Directory('test/output/tvz');
    if (!outDir.existsSync()) outDir.createSync(recursive: true);
    File('test/output/tvz/${label}_3games.md').writeAsStringSync(buf.toString());
  }
  ...
  Future<void> run1000Stats(String homeBuild, String awayBuild, String label) async {
    ...
    final buf = StringBuffer();
    buf.writeln('# TvZ $label 1000경기 통계');
    buf.writeln();
    buf.writeln('| 항목 | 값 |');
    buf.writeln('|------|-----|');
    buf.writeln('| 총 경기 | $totalGames |');
    buf.writeln('| T 승률 | $homeWinRate% ($homeWins승) |');
    buf.writeln('| Z 승률 | ${(100 - double.parse(homeWinRate)).toStringAsFixed(1)}% (${totalGames - homeWins}승) |');
    buf.writeln('| 평균 로그 길이 | $avgLogLen줄 |');
    buf.writeln('| 평균 최종 병력 | T $avgHomeArmy / Z $avgAwayArmy |');
    ...
    final outDir = Directory('test/output/tvz');
    if (!outDir.existsSync()) outDir.createSync(recursive: true);
    File('test/output/tvz/${label}_1000stats.md').writeAsStringSync(buf.toString());
    print('$label: T승률 $homeWinRate% | 초${(earlyEnd/10).toStringAsFixed(0)} 중${(midEnd/10).toStringAsFixed(0)} 후${(lateEnd/10).toStringAsFixed(0)} | 고유${uniqueTexts.length}');
  }
