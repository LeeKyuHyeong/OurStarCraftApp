...
/// PvP 시나리오 5-8 종합 검증 (300게임 x 12설정)
void main() {
  ...
  group('PvP S5: 질럿러시 vs 스탠다드', () {
    ...
    tearDownAll(() {
      final report = generateFullReport(
        title: 'S5 질럿러시 vs 스탠다드',
        results: results,
        phaseName2: 'Phase 2 러시 결과',
      );
      File('test/output/pvp_s5_report.md').writeAsStringSync(report);
    });
  ...
  // S6: 다크 vs 질럿러시
  // =======================================================
  group('PvP S6: 다크 vs 질럿러시', () {
    const build1 = 'pvp_dark_allin';
    const build2 = 'pvp_zealot_rush';
    final results = <String, BatchResult>{};
    ...
    tearDownAll(() {
      final report = generateFullReport(
        title: 'S6 다크 vs 질럿러시',
        results: results,
        phaseName2: 'Phase 2 치즈 결과',
      );
      File('test/output/pvp_s6_report.md').writeAsStringSync(report);
    });
  ...
  group('PvP S7: 로보틱스 미러', () {
    ...
    tearDownAll(() {
      final report = generateFullReport(
        title: 'S7 로보틱스 미러',
        results: results,
        phaseName2: 'Phase 2 리버 교환',
        phaseName4: 'Phase 4 결전',
      );
      File('test/output/pvp_s7_report.md').writeAsStringSync(report);
    });
  ...
  // S8: 4게이트 vs 원겟멀티
  // =======================================================
  group('PvP S8: 4게이트 vs 원겟멀티', () {
    const build1 = 'pvp_4gate_dragoon';
    const build2 = 'pvp_1gate_multi';
    final results = <String, BatchResult>{};

    tearDownAll(() {
      final report = generateFullReport(
        title: 'S8 4게이트 vs 원겟멀티',
        results: results,
        phaseName2: 'Phase 2 압박 결과',
      );
      File('test/output/pvp_s8_report.md').writeAsStringSync(report);
    });
    ...
    test('등급차 소 P1 A vs P2 B', () async {
      final r = await runBatchBiDir(player1: homeAGrade, player2: awayBGrade, map: mapBalance, build1: build1, build2: build2, countPerDir: 150, makeBranches2: s8Phase2);
      results['등급차 소 P1 A vs P2 B'] = r;
      expect(r.homeWinRate, greaterThan(40));
    });
    test('등급차 소 P2 A vs P1 B', () async {
      final r = await runBatchBiDir(player1: homeBGrade, player2: awayAGrade, map: mapBalance, build1: build1, build2: build2, countPerDir: 150, makeBranches2: s8Phase2);
      results['등급차 소 P2 A vs P1 B'] = r;
      expect(r.homeWinRate, lessThan(60));
    });
    test('등급차 중 P1 S vs P2 B', () async {
      final r = await runBatchBiDir(player1: homeSGrade, player2: awayBGrade, map: mapBalance, build1: build1, build2: build2, countPerDir: 150, makeBranches2: s8Phase2);
      results['등급차 중 P1 S vs P2 B'] = r;
      expect(r.homeWinRate, greaterThan(55));
    });
    test('등급차 중 P2 S vs P1 B', () async {
      final r = await runBatchBiDir(player1: homeBGrade, player2: awaySGrade, map: mapBalance, build1: build1, build2: build2, countPerDir: 150, makeBranches2: s8Phase2);
      results['등급차 중 P2 S vs P1 B'] = r;
      expect(r.homeWinRate, lessThan(45));
    });
    test('등급차 대 P1 S+ vs P2 B', () async {
      final r = await runBatchBiDir(player1: homeSPlus, player2: awayBGrade, map: mapBalance, build1: build1, build2: build2, countPerDir: 150, makeBranches2: s8Phase2);
      results['등급차 대 P1 S+ vs P2 B'] = r;
      expect(r.homeWinRate, greaterThan(60));
    });
    test('등급차 대 P2 S+ vs P1 B', () async {
      final r = await runBatchBiDir(player1: homeBGrade, player2: awaySPlus, map: mapBalance, build1: build1, build2: build2, countPerDir: 150, makeBranches2: s8Phase2);
      results['등급차 대 P2 S+ vs P1 B'] = r;
      expect(r.homeWinRate, lessThan(40));
    });

    test('특화 공격 vs 수비', () async {
      final r = await runBatchBiDir(player1: homeAttack, player2: awayDefense, map: mapBalance, build1: build1, build2: build2, countPerDir: 150, makeBranches2: s8Phase2);
      results['특화 공격 vs 수비'] = r;
      expect(r.homeWinRate, greaterThan(10));
      expect(r.homeWinRate, lessThan(90));
    });
    test('특화 수비 vs 공격', () async {
      final r = await runBatchBiDir(player1: homeDefense, player2: awayAttack, map: mapBalance, build1: build1, build2: build2, countPerDir: 150, makeBranches2: s8Phase2);
      results['특화 수비 vs 공격'] = r;
      expect(r.homeWinRate, greaterThan(10));
      expect(r.homeWinRate, lessThan(90));
    });
