/// PvT 전체 63개 시나리오 3경기 로그 + 1000경기 통계
import 'dart:io';
...
void main() {
  final homePlayer = Player(
    id: 'protoss_home', name: '김택용', raceIndex: 2,
  ...
  final awayPlayer = Player(
    id: 'terran_away', name: '이영호', raceIndex: 0,
    stats: const PlayerStats(
      sense: 700, control: 700, attack: 700, harass: 700,
      strategy: 700, macro: 700, defense: 700, scout: 700,
    ),
  ...
  for (final h in homeBuilds) {
    for (final a in awayBuilds) {
      final hLabel = h.replaceFirst('pvt_trans_', '').replaceFirst('pvt_', '');
      final aLabel = a.replaceFirst('tvp_trans_', '').replaceFirst('tvp_', '');
  ...
  Future<void> run3Games(String homeBuild, String awayBuild, String label) async {
    ...
    final outDir = Directory('test/output/pvt');
    if (!outDir.existsSync()) outDir.createSync(recursive: true);
    File('test/output/pvt/${label}_3games.md').writeAsStringSync(buf.toString());
  }
  ...
  Future<void> run1000Stats(String homeBuild, String awayBuild, String label) async {
    ...
    final buf = StringBuffer();
    buf.writeln('# PvT $label 1000경기 통계');
    buf.writeln();
    buf.writeln('| 항목 | 값 |');
    buf.writeln('|------|-----|');
    ...
    final outDir = Directory('test/output/pvt');
    if (!outDir.existsSync()) outDir.createSync(recursive: true);
    File('test/output/pvt/${label}_1000stats.md').writeAsStringSync(buf.toString());
