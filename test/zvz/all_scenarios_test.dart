...
void main() {
  ...
  for (int i = 0; i < builds.length; i++) {
    for (int j = i; j < builds.length; j++) {
      scenarios.add({
        ...
        'label': i == j
            ? '${builds[i].replaceFirst("zvz_", "")}_mirror'
            : '${builds[i].replaceFirst("zvz_", "")}_vs_${builds[j].replaceFirst("zvz_", "")}',
      });
    }
  }
  ...
  Future<void> run3Games(String homeBuild, String awayBuild, String label) async {
    ...
    File('test/output/zvz/${label}_3games.md').writeAsStringSync(buf.toString());
  }
  ...
  Future<void> run1000Stats(String homeBuild, String awayBuild, String label) async {
    ...
    final buf = StringBuffer();
    buf.writeln('# ZvZ $label 1000경기 통계');
    buf.writeln();
    buf.writeln('| 항목 | 값 |');
    buf.writeln('|------|-----|');
    ...
    final outDir = Directory('test/output/zvz');
    if (!outDir.existsSync()) outDir.createSync(recursive: true);
    File('test/output/zvz/${label}_1000stats.md').writeAsStringSync(buf.toString());
