...
/// ============================================================
/// PvP 1경기 테스트 - 빌드 ID를 바꿔가며 개별 시나리오 확인용
/// ============================================================
void main() {
  // ── 빌드 ID 설정 (여기만 수정) ──
  const homeBuildId = 'pvp_2gate_dragoon';
  const awayBuildId = 'pvp_1gate_robo';
  ...
  test('PvP 1경기: $homeBuildId vs $awayBuildId', () async {
    ...
    buf.writeln('**선수**: ${homePlayer.name}(프로토스/홈) vs ${awayPlayer.name}(프로토스/어웨이)');
    ...
    File('test/output/pvp/single_log.md').writeAsStringSync(buf.toString());
    print(buf.toString());
  });
}
