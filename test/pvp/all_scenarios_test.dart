/// PvP 전체 36개 시나리오 3경기 로그 + 1000경기 통계
import 'dart:io';
...
void main() {
  final homePlayer = Player(
    id: 'protoss_home', name: '홍진호', raceIndex: 2,
  ...
  final awayPlayer = Player(
    id: 'protoss_away', name: '이제동', raceIndex: 2,
    stats: const PlayerStats(
      sense: 700, control: 700, attack: 700, harass: 700,
      strategy: 700, macro: 700, defense: 700, scout: 700,
    ),
  ...
  for (int i = 0; i < builds.length; i++) {
    for (int j = i; j < builds.length; j++) {
      scenarios.add({
        ...
        'label': i == j
            ? '${builds[i].replaceFirst("pvp_", "")}_mirror'
            : '${builds[i].replaceFirst("pvp_", "")}_vs_${builds[j].replaceFirst("pvp_", "")}',
      });
    }
  }
  ...
  Future<void> run3Games(String homeBuild, String awayBuild, String label) async {
    ...
    final outDir = Directory('test/output/pvp');
    if (!outDir.existsSync()) outDir.createSync(recursive: true);
    File('test/output/pvp/${label}_3games.md').writeAsStringSync(buf.toString());
  }
  ...
  Future<void> run1000Stats(String homeBuild, String awayBuild, String label) async {
    ...
    final buf = StringBuffer();
    buf.writeln('# PvP $label 1000경기 통계');
    buf.writeln();
    buf.writeln('| 항목 | 값 |');
    buf.writeln('|------|-----|');
    ...
    final outDir = Directory('test/output/pvp');
    if (!outDir.existsSync()) outDir.createSync(recursive: true);
    File('test/output/pvp/${label}_1000stats.md').writeAsStringSync(buf.toString());
