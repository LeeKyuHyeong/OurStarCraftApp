...
class BuildOrderData {
  ...
  // ==================== 명명 상수 ====================
  static const int _lowResourceThreshold = 100;
  static const int _lowArmyThreshold = 60;
  static const int _midGameLine = 50;
  static const int _lateGameLine = 100;

  // ==================== 유닛 태그 캐시 ====================
  static final Map<String, Set<String>> _unitTagCache = {};
  ...
  static const zergVsTerranBuilds = [
    ...
    BuildOrder(
      ...
      steps: [
        ...
        BuildStep(line: 39, text: '{player} 선수 퀸즈 네스트 건설!', myResource: -15),
    ...
    BuildOrder(
      ...
      steps: [
        ...
        BuildStep(line: 68, text: '{player} 선수 퀸즈 네스트 건설!', myResource: -15),
  ...
  static const zergVsProtossBuilds = [
    ...
    BuildOrder(
      ...
      steps: [
        ...
        BuildStep(line: 56, text: '{player} 선수 퀸즈 네스트 건설!', myResource: -15),
  ...
  static const highHarassEvents = [
    ...
    ClashEvent(
      text: '{attacker} 선수, 일꾼 연속 견제! 일꾼 수를 줄여줍니다!',
      favorsStat: 'harass',
      attackerArmy: -5,
      defenderArmy: -5,
      defenderResource: -50,
  ...
  static const Map<BuildType, List<ClashEvent>> buildTypeEvents = {
    ...
    BuildType.zvt1HatchAllIn: [
      ClashEvent(text: '원해처리 럴커! 상대 앞마당 입구에 박힙니다!', favorsStat: 'attack', attackerArmy: -3, defenderArmy: -8, defenderResource: -20),
      ClashEvent(text: '럴커 올인! 스캔 전에 바이오닉이 녹습니다!', favorsStat: 'strategy', attackerArmy: -2, defenderArmy: -10, defenderResource: -15),
    ],
    ...
    BuildType.zvp2HatchMutal: [
      ClashEvent(text: '5뮤탈 견제! 프로브 공격!', favorsStat: 'harass', attackerArmy: -2, defenderArmy: -6, defenderResource: -30),
      ClashEvent(text: '뮤탈 5기로 맵 장악! 히드라 전환 준비!', favorsStat: 'strategy', attackerArmy: -1, defenderArmy: -8, defenderResource: -20),
    ],
    ...
    BuildType.zvpMukerji: [
      ClashEvent(text: '뮤커지! 뮤탈 스커지 공중 장악!', favorsStat: 'harass', attackerArmy: -2, defenderArmy: -10, defenderResource: -20),
      ClashEvent(text: '뮤탈 스커지 조합! 커세어 격추 후 견제!', favorsStat: 'control', attackerArmy: -3, defenderArmy: -8, defenderResource: -25),
    ],
    ...
    BuildType.pvtProxyGate: [
      ClashEvent(text: '센터 게이트 질럿! SCV 직행!', favorsStat: 'attack', attackerArmy: -6, defenderArmy: -10, defenderResource: -20),
    ...
    BuildType.pvz2GateZealot: [
      ClashEvent(text: '전진 2게이트! 질럿으로 앞마당 압박!', favorsStat: 'attack', attackerArmy: -8, defenderArmy: -10, defenderResource: -15),
      ClashEvent(text: '질럿 추가 투입! 성큰 완성 전에 밀어붙입니다!', favorsStat: 'control', attackerArmy: -6, defenderArmy: -12),
    ],
    ...
    BuildType.pvpZealotRush: [
      ClashEvent(text: '센터99게이트! 프로브 직행!', favorsStat: 'attack', attackerArmy: -8, defenderArmy: -10, defenderResource: -15),
    ...
    BuildType.pvp2GateReaver: [
      ClashEvent(text: '투겟 리버! 셔틀에 리버를 태워 드랍!', favorsStat: 'harass', attackerArmy: -3, defenderArmy: -8, defenderResource: -35),
      ClashEvent(text: '리버 스캐럽 작렬! 프로브 초토화!', favorsStat: 'control', attackerArmy: -2, defenderArmy: -6, defenderResource: -40),
    ],
    BuildType.pvp3GateSpeedZealot: [
      ClashEvent(text: '발업 질럿! 다리 업 질럿이 밀려옵니다!', favorsStat: 'attack', attackerArmy: -6, defenderArmy: -10, defenderResource: -15),
      ClashEvent(text: '질럿 물량 압박! 드라군이 밀립니다!', favorsStat: 'control', attackerArmy: -5, defenderArmy: -12),
    ],
  };
  ...
  static const aggressiveVsDefensive = [
    ...
    // 추가 이벤트
    ClashEvent(
      text: '{defender}, 완벽한 진형 유지! 공격 흡수!',
      favorsStat: 'macro',
      attackerArmy: -8,
      defenderArmy: -3,
    ),
    ClashEvent(
      text: '{attacker}, 측면 우회 공격 성공! 수비 라인 붕괴!',
      favorsStat: 'sense',
      attackerArmy: -4,
      defenderArmy: -12,
      defenderResource: -20,
    ...
    ClashEvent(
      text: '{defender}, 수비 진지에서 카운터 타이밍! 역공 개시!',
      favorsStat: 'strategy',
      attackerArmy: -12,
      defenderArmy: -5,
    ),
    ClashEvent(
      text: '{attacker}, 멀티 방면 동시 압박! {defender} 수비 한계!',
      favorsStat: 'control',
      attackerArmy: -6,
      defenderArmy: -10,
      defenderResource: -15,
  ...
  static const aggressiveVsAggressive = [
    ...
    // 추가 이벤트
    ClashEvent(
      text: '{attacker}, 병력 집결 후 일점 돌파! 전선 뚫립니다!',
      favorsStat: 'attack',
      attackerArmy: -8,
      defenderArmy: -15,
    ),
    ClashEvent(
      text: '양측 주력끼리 정면 교환! 치열한 소모전!',
      favorsStat: 'macro',
      attackerArmy: -12,
      defenderArmy: -10,
    ),
    ClashEvent(
      text: '{defender}, 읽기 싸움 승리! 카운터 병력 투입!',
      favorsStat: 'scout',
      attackerArmy: -15,
      defenderArmy: -3,
    ),
    ClashEvent(
      text: '{attacker}, 기습 각도 진입! {defender} 대응 늦어!',
      favorsStat: 'sense',
      attackerArmy: -5,
      defenderArmy: -12,
    ),
  ...
  static const dropEventsTerran = [
    ClashEvent(
      text: '{attacker} 선수, 드랍십 투입! 일꾼 초토화!',
      favorsStat: 'harass',
      attackerArmy: -3,
      defenderArmy: -5,
      defenderResource: -40,
  ...
  static const spellEventsGeneral = [
    ...
    ClashEvent(
      text: '{attacker}, 핵심 스킬로 상대 주력 무력화!',
      favorsStat: 'sense',
      attackerArmy: -3,
      defenderArmy: -10,
    ),
    ClashEvent(
      text: '스킬 타이밍 완벽! {defender} 진형 순간 와해!',
      favorsStat: 'strategy',
      attackerArmy: -4,
      defenderArmy: -12,
      defenderResource: -15,
  ...
  static const comebackEvents = [
    ...
    // 추가 이벤트
    ClashEvent(
      text: '{defender}, 소수 병력으로 기적같은 방어! 시간 벌기 성공!',
      favorsStat: 'defense',
      attackerArmy: -12,
      defenderArmy: -5,
    ),
    ClashEvent(
      text: '{defender}, 확장 자원으로 병력 재건! 역전의 발판!',
      favorsStat: 'macro',
      attackerArmy: -10,
      defenderArmy: -3,
      attackerResource: -15,
  ...
  static const tensionEvents = [
    ...
    // epicMomentEvents/amazingPlayEvents 통합
    ClashEvent(
  ...
  static const comebackDramaEvents = [
    ...
    // epicMomentEvents/amazingPlayEvents 통합
    ClashEvent(
      text: '믿을 수가 없습니다! {defender} 선수의 신의 한수!',
      favorsStat: 'sense',
      attackerArmy: -15,
      defenderArmy: -5,
      decisive: true,
  ...
  static const microEventsGeneral = [
    ...
    // 추가 이벤트
    ClashEvent(
      text: '유닛 빼기 성공! 피해 입은 유닛 후열로 이동!',
      favorsStat: 'control',
      attackerArmy: -2,
      defenderArmy: -8,
    ),
    ClashEvent(
      text: '포커싱 대결! {attacker} 선수 한 끗 차이로 승리!',
      favorsStat: 'sense',
      attackerArmy: -4,
      defenderArmy: -10,
    ),
    ClashEvent(
      text: '진형 전환! 상대 예측 빗나가며 교전 우위!',
      favorsStat: 'strategy',
      attackerArmy: -3,
      defenderArmy: -10,
    ),
  ...
  static const mindGameEvents = [
    ...
    // 추가 이벤트
    ClashEvent(
      text: '{attacker}, 페이크 공격으로 상대 병력 유인 성공!',
      favorsStat: 'strategy',
      attackerArmy: -3,
      defenderArmy: -8,
      defenderResource: -15,
    ...
    ClashEvent(
      text: '{defender}, 정찰로 기습 타이밍 파악! 완벽 대비!',
      favorsStat: 'sense',
      attackerArmy: -10,
      defenderArmy: -2,
    ),
    ClashEvent(
      text: '{attacker}, 테크 전환으로 상대 허를 찌릅니다!',
      favorsStat: 'strategy',
      attackerArmy: -2,
      defenderArmy: -10,
    ),
  ...
  static const clashEventsZvZ = [
    ...
    ClashEvent(
      text: '히드라 생산! 뮤탈 전 지상 압박!',
      favorsStat: 'strategy',
      attackerArmy: -5,
      defenderArmy: -8,
    ),
    ClashEvent(
      text: '저글링 속업! 속도 차이로 포위 성공!',
      favorsStat: 'attack',
      attackerArmy: -6,
      defenderArmy: -10,
    ),
  ...
  static const skirmishEvents = [
    ...
    BuildStep(line: 0, text: '{player}, 상대 일꾼 견제!', stat: 'harass', enemyResource: -20),
    ...
    BuildStep(line: 0, text: '{player}, 상대 일꾼 타격!', stat: 'harass', myArmy: -2, enemyResource: -25),
  ...
  static const zergMidLateEvents = [
    ...
    BuildStep(line: 0, text: '{player}, 디파일러 에너지 업그레이드 완료!', stat: 'strategy', myResource: -15),
    BuildStep(line: 0, text: '{player} 선수 가디언 변태!', stat: 'strategy', myArmy: 4, myResource: -20),
    BuildStep(line: 0, text: '{player}, 디바우러 변태! 적진 잠식!', stat: 'strategy', myArmy: 3, myResource: -18),
    BuildStep(line: 0, text: '{player} 선수 스커지 생산!', stat: 'attack', myArmy: 3, myResource: -10),
    BuildStep(line: 0, text: '{player}, 저글링 속업 완료!', stat: 'control', myArmy: 2, myResource: -10),
    BuildStep(line: 0, text: '{player} 선수 해처리 추가 건설!', stat: 'macro', myArmy: 2, myResource: -15),
    BuildStep(line: 0, text: '{player}, 오버로드 속업 완료!', stat: 'scout', myArmy: 1, myResource: -10),
  ...
  static const protossMidLateEvents = [
    ...
    BuildStep(line: 0, text: '{player} 선수 아콘 합체!', stat: 'strategy', myArmy: 4, myResource: -15),
    ...
    BuildStep(line: 0, text: '{player}, 커세어 추가 생산!', stat: 'control', myArmy: 2, myResource: -15),
  ...
  static const tvzMidLateEvents = [
    ...
    BuildStep(line: 0, text: '{player} 선수 탱크 시즈모드로 럴커 사냥!', stat: 'strategy', myArmy: -1, enemyArmy: -3),
    BuildStep(line: 0, text: '{player}, 베슬 이레디에이트 적중!', stat: 'strategy', enemyArmy: -4),
    BuildStep(line: 0, text: '{player} 선수 드랍십으로 본진 기습!', stat: 'harass', myArmy: -2, enemyResource: -30),
    ...
    // 확장 이벤트
    BuildStep(line: 0, text: '{player}, 벌처 마인으로 확장 저지!', stat: 'harass', myArmy: -1, enemyResource: -25),
    BuildStep(line: 0, text: '{player} 선수 터렛 건설! 뮤탈 접근 차단!', stat: 'defense', myArmy: 1, myResource: -12),
    BuildStep(line: 0, text: '{player}, 드랍십 2기 동시 기습! 상대 멀티 초토화!', stat: 'attack', myArmy: -3, enemyResource: -35),
    BuildStep(line: 0, text: '{player} 선수 탱크 한 줄 시즈! 저글링 접근 불가!', stat: 'strategy', myArmy: 2, myResource: -18),
    BuildStep(line: 0, text: '{player}, 스캔으로 럴커 위치 파악!', stat: 'scout', enemyArmy: -2),
    BuildStep(line: 0, text: '{player} 선수 커맨드센터 추가 건설!', stat: 'macro', myArmy: 2, myResource: -20),
    BuildStep(line: 0, text: '{player}, 골리앗 라인으로 가디언 견제!', stat: 'defense', myArmy: -1, enemyArmy: -3),
    BuildStep(line: 0, text: '{player} 선수 마린 메딕 조합으로 전진!', stat: 'attack', myArmy: 3, myResource: -15),
  ];
  ...
  static const tvpMidLateEvents = [
    ...
    BuildStep(line: 0, text: '{player}, 드랍십으로 프로브 기습!', stat: 'harass', myArmy: -2, enemyResource: -25),
    ...
    BuildStep(line: 0, text: '{player} 선수 마린 메딕으로 드라군 교환!', stat: 'control', myArmy: -3, enemyArmy: -4),
    BuildStep(line: 0, text: '{player}, 아머리 업그레이드로 골리앗 강화!', stat: 'macro', myArmy: 2, myResource: -15),
    BuildStep(line: 0, text: '{player} 선수 고스트 락다운으로 아비터 무력화!', stat: 'sense', enemyArmy: -4),
  ];
  ...
  static const tvtMidLateEvents = [
    ...
    BuildStep(line: 0, text: '{player}, 골리앗 생산! 대공 대비!', stat: 'defense', myArmy: 3, myResource: -15),
    BuildStep(line: 0, text: '{player} 선수 골리앗으로 레이스 요격!', stat: 'defense', myArmy: -1, enemyArmy: -3),
    // 확장 이벤트
  ...
  static const zvtMidLateEvents = [
    BuildStep(line: 0, text: '{player}, 뮤탈로 SCV 견제!', stat: 'harass', myArmy: -1, enemyResource: -25),
    ...
    // 확장 이벤트
    BuildStep(line: 0, text: '{player}, 럴커 매복으로 마린 메딕 궤멸!', stat: 'defense', myArmy: -1, enemyArmy: -5),
    BuildStep(line: 0, text: '{player} 선수 오버로드 정찰! 상대 드랍 루트 파악!', stat: 'scout', myArmy: 1),
    BuildStep(line: 0, text: '{player}, 저글링으로 상대 커맨드센터 견제!', stat: 'harass', myArmy: -2, enemyResource: -25),
    BuildStep(line: 0, text: '{player} 선수 디파일러 플레이그로 마린 녹이기!', stat: 'strategy', enemyArmy: -5),
    BuildStep(line: 0, text: '{player}, 뮤탈 견제로 상대 멀티 일꾼 타격!', stat: 'harass', myArmy: -1, enemyResource: -20),
    BuildStep(line: 0, text: '{player} 선수 성큰으로 벌처 진입 차단!', stat: 'defense', myArmy: 1, myResource: -12),
    BuildStep(line: 0, text: '{player}, 스커지로 드랍십 격추! 기습 차단!', stat: 'sense', myArmy: -1, enemyArmy: -3),
    BuildStep(line: 0, text: '{player} 선수 울트라 디파일러 조합! 최종 병기!', stat: 'attack', myArmy: 4, myResource: -25),
  ];
  ...
  static const zvpMidLateEvents = [
    ...
    BuildStep(line: 0, text: '{player}, 스커지로 커세어 격추!', stat: 'attack', myArmy: -2, enemyArmy: -2),
    BuildStep(line: 0, text: '{player} 선수 뮤탈로 프로브 급습!', stat: 'harass', myArmy: -1, enemyResource: -25),
    ...
    // 확장 이벤트
    BuildStep(line: 0, text: '{player}, 오버로드 정찰로 캐논 배치 확인!', stat: 'scout', myArmy: 1),
    BuildStep(line: 0, text: '{player} 선수 해처리 추가! 물량 경쟁 돌입!', stat: 'macro', myArmy: 3, myResource: -15),
    BuildStep(line: 0, text: '{player}, 뮤탈로 상대 확장 넥서스 견제!', stat: 'harass', myArmy: -1, enemyResource: -25),
    BuildStep(line: 0, text: '{player} 선수 저글링으로 상대 멀티 프로브 급습!', stat: 'harass', myArmy: -3, enemyResource: -20),
    BuildStep(line: 0, text: '{player}, 히드라 물량으로 리버 셔틀 격추!', stat: 'defense', myArmy: -2, enemyArmy: -4),
    BuildStep(line: 0, text: '{player} 선수 디파일러 다크스웜으로 드라군 무력화!', stat: 'strategy', myArmy: 2, enemyArmy: -3),
    BuildStep(line: 0, text: '{player}, 울트라로 캐논 라인 돌파!', stat: 'attack', myArmy: -2, enemyArmy: -4),
    BuildStep(line: 0, text: '{player} 선수 스커지로 커세어 격추! 제공권 탈환!', stat: 'control', myArmy: -2, enemyArmy: -3),
  ];
  ...
  static const zvzMidLateEvents = [
    ...
    BuildStep(line: 0, text: '{player}, 뮤탈로 상대 드론 급습!', stat: 'harass', myArmy: -1, enemyResource: -25),
    ...
    // Liquipedia 기반 추가 이벤트
    BuildStep(line: 0, text: '{player}, 뮤탈밤! 드론 집중 타격!', stat: 'harass', enemyResource: -30),
    BuildStep(line: 0, text: '{player} 선수 스커지 분산! 뮤탈 2기에 동시 피해!', stat: 'strategy', myArmy: -2, enemyArmy: -4),
    BuildStep(line: 0, text: '{player}, 저글링으로 상대 스파이어 공격!', stat: 'attack', myArmy: -3, enemyArmy: -2),
    BuildStep(line: 0, text: '{player} 선수 성큰 콜로니 급조! 저글링 돌격 차단!', stat: 'defense', myArmy: 1, myResource: -12),
    BuildStep(line: 0, text: '{player}, 스포어 콜로니 건설! 뮤탈 접근 차단!', stat: 'defense', myArmy: 1, myResource: -10),
    BuildStep(line: 0, text: '{player} 선수 뮤탈 체력 관리! 피해 입은 뮤탈 후퇴!', stat: 'control', myArmy: -1, enemyArmy: -2),
    BuildStep(line: 0, text: '{player}, 저글링으로 확장 해처리 견제!', stat: 'harass', myArmy: -2, enemyResource: -20),
    BuildStep(line: 0, text: '{player} 선수 드론으로 저글링 방어! 미네랄 워크!', stat: 'defense', myArmy: 1, enemyArmy: -2),
    // 추가 이벤트
    BuildStep(line: 0, text: '{player}, 뮤탈 스택으로 히드라 한 기 집중 사격!', stat: 'attack', myArmy: -1, enemyArmy: -3),
    BuildStep(line: 0, text: '{player} 선수 오버로드 스카우팅! 상대 스파이어 타이밍 파악!', stat: 'sense', myArmy: 1),
  ];
  ...
  static const pvtMidLateEvents = [
    ...
    BuildStep(line: 0, text: '{player}, 셔틀 리버로 본진 기습!', stat: 'harass', myArmy: -1, enemyResource: -25),
    ...
    BuildStep(line: 0, text: '{player}, 셔틀 다크템플러로 상대 멀티 기습!', stat: 'harass', myArmy: -1, enemyResource: -30),
    BuildStep(line: 0, text: '{player} 선수 드라군 포커싱으로 탱크 저격!', stat: 'control', myArmy: -2, enemyArmy: -4),
    BuildStep(line: 0, text: '{player}, 스톰으로 마린 메딕 뭉치 초토화!', stat: 'strategy', myArmy: -1, enemyArmy: -5),
    BuildStep(line: 0, text: '{player} 선수 옵저버로 마인 위치 탐지!', stat: 'scout', myArmy: 1),
  ];

  /// PvZ 매치업 이벤트 (프로토스 시점)
  static const pvzMidLateEvents = [
    BuildStep(line: 0, text: '{player}, 커세어로 오버로드 사냥!', stat: 'control', enemyArmy: -2),
    ...
    BuildStep(line: 0, text: '{player}, 아콘으로 저글링 물량 녹이기!', stat: 'attack', myArmy: -1, enemyArmy: -5),
    BuildStep(line: 0, text: '{player} 선수 캐리어 전환 시작!', stat: 'macro', myArmy: 4, myResource: -30),
    // 확장 이벤트
    BuildStep(line: 0, text: '{player}, 넥서스 추가! 서드 확장 성공!', stat: 'macro', myArmy: 2, myResource: -20),
    BuildStep(line: 0, text: '{player} 선수 다크템플러로 드론 학살!', stat: 'harass', enemyArmy: -2, enemyResource: -25),
    BuildStep(line: 0, text: '{player}, 질럿 레그로 저글링 교환 승리!', stat: 'attack', myArmy: -2, enemyArmy: -5),
    BuildStep(line: 0, text: '{player} 선수 옵저버로 럴커 위치 파악!', stat: 'scout', myArmy: 1),
    BuildStep(line: 0, text: '{player}, 셔틀 리버로 상대 확장 해처리 타격!', stat: 'harass', myArmy: -1, enemyResource: -30),
    BuildStep(line: 0, text: '{player} 선수 아콘으로 히드라 물량 소각!', stat: 'attack', myArmy: -2, enemyArmy: -5),
    BuildStep(line: 0, text: '{player}, 드라군으로 뮤탈 요격!', stat: 'defense', myArmy: -1, enemyArmy: -3),
    BuildStep(line: 0, text: '{player} 선수 캐논 추가로 럴커 접근 차단!', stat: 'defense', myArmy: 1, myResource: -12),
  ];
  ...
  static const pvpMidLateEvents = [
    ...
    BuildStep(line: 0, text: '{player} 선수 커세어로 제공권 장악!', stat: 'control', myArmy: 2, myResource: -15),
    // 하이템플러 스톰
    BuildStep(line: 0, text: '{player} 선수 하이템플러 사이오닉 스톰!', stat: 'strategy', enemyArmy: -5, myResource: -15),
    BuildStep(line: 0, text: '{player}, 스톰으로 드라군 뭉치 녹이기!', stat: 'strategy', myArmy: -1, enemyArmy: -6),
    // 아콘 합체/전투
    ...
    // 셔틀 리버 멀티 견제
    BuildStep(line: 0, text: '{player}, 셔틀 리버로 상대 멀티 기습!', stat: 'harass', myArmy: -1, enemyArmy: -3, enemyResource: -25),
    // 셔틀 하이템플러 본진 견제
    BuildStep(line: 0, text: '{player}, 셔틀 하이템플러로 상대 본진 기습! 스톰 작렬!', stat: 'strategy', myArmy: -1, enemyArmy: -4, enemyResource: -20),
    // 추가 이벤트
    BuildStep(line: 0, text: '{player} 선수 넥서스 추가! 확장 우위 확보!', stat: 'macro', myArmy: 2, myResource: -20),
    BuildStep(line: 0, text: '{player}, 캐논으로 다크템플러 진입 차단!', stat: 'defense', myArmy: 1, myResource: -12),
    BuildStep(line: 0, text: '{player} 선수 옵저버 배치! 상대 움직임 포착!', stat: 'sense', myArmy: 1),
  ];
  ...
  /// 동일 빌드 재스캔 방지를 위해 캐싱 적용 (O(n*m) → O(1))
  static Set<String> extractUnitTags(BuildOrder build) {
    return _unitTagCache.putIfAbsent(build.id, () {
      final tags = <String>{};
  ...
  static BuildStep getMidLateEvent({
    ...
    if (currentResource < _lowResourceThreshold) {
    ...
    // 병력이 낮으면 생산 이벤트 우선
    else if (currentArmy < _lowArmyThreshold) {
    ...
    // 중반 (50줄 이전)
    else if (lineCount < _midGameLine) {
      candidates.addAll(expansionEvents);
      candidates.addAll(productionEvents);
    ...
    // 중후반 (50~100줄)
    else if (lineCount < _lateGameLine) {
      candidates.addAll(productionEvents);
      candidates.addAll(skirmishEvents);
