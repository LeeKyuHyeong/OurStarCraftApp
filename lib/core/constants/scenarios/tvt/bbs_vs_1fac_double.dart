...
// ----------------------------------------------------------
// BBS 올인 러시 vs 원팩 확장
// 타이밍: BBS 첫 마린 2:10, 마린 3기 ~2:25 / 원팩더블 첫 마린 2:23, 팩토리 가스 100 도달
//
...
//
// 센터정찰: 75% 수비 우세 (def_wins) / 25% BBS 압박 (atk_wins)
// 본진정찰: 75% BBS 큰 피해 (bunker_success) / 25% SCV로 막음 (defended)
//
...
const _tvtBbsVs1facDouble = ScenarioScript(
  ...
  phases: [
    ...
    // ── Phase 1: 수비자 정찰 + 교전 (NON-decisive, lines 9-15) ────
    // baseProbability로 75/25 비율 제어, army state로 Phase 2 결정
    ScriptPhase(
      ...
      recoveryArmyPerLine: 0,
      branches: [
        // ─── A1: 센터 정찰 + SCV 컨트롤 → 수비 우세 (75% of center) ───
        ScriptBranch(
          id: 'center_def_wins',
          ...
          events: [
            ...
            ScriptEvent(
              text: '{away} 선수 추가 벌처를 상대방 진영으로 보냅니다. 사실상 bbs가 막힌순간 승기를 잡았죠.',
              owner: LogOwner.away,
              awayArmy: 6,
              awayResource: -75,
              fixedCost: true,
        ...
        // ─── A2: 센터 정찰 + BBS 컨트롤 우위 → BBS 우세 (25% of center) ───
        ScriptBranch(
          id: 'center_atk_wins',
        ...
        // ─── B1: 본진 정찰 + 1vs3 + 벙커 성공 → BBS 큰 피해 (75% of base) ───
        ScriptBranch(
        ...
        // ─── B2: 본진 정찰이지만 SCV 컨트롤로 막음 → 수비 우세 (25% of base) ───
        ScriptBranch(
          id: 'base_defended',
    ...
    // ── Phase 2: 결정타 (lines 17-18) ────────────────────────
    // army state shortcut: totalArmy > 10일 때 homeRatio > 0.85 → home, < 0.15 → away
    ScriptPhase(
      name: 'finisher',
      startLine: 17,
