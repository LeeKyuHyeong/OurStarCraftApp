...
6. **용어 규칙**: 시나리오 텍스트에서 '경제'라는 단어를 사용하지 않음. '자원', '일꾼', '확장' 등 구체적 표현 사용
...
- **homeBuildIds와 awayBuildIds는 겹치면 안 됨**: 같은 빌드가 양쪽 리스트에 있으면 역방향 매칭(`isReversed`)이 작동하지 않아 홈/어웨이 반전 시 승률 편향 발생
- 역방향 매칭 원리: 빌드 리스트가 겹치지 않으면, 빌드가 반대로 들어왔을 때 Phase 1(정방향)이 실패하고 Phase 2(역방향)에서 매칭 → `isReversed=true` → army/resource/text/owner 전부 자동 스왑
- homeBuildIds에는 스크립트의 `{home}` 내러티브에 맞는 빌드만, awayBuildIds에는 `{away}` 내러티브에 맞는 빌드만 넣을 것
