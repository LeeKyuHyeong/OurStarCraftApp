...
### 개별 시나리오 사전 루프 (test_{matchup}.md 'Y' 처리 전)
...
C. 판단
   - 위 값 만족 + 사용자 육안 검수 OK → test_{matchup}.md 'Y' 처리
...
5. 결과 파일 생성
   ...
   - C13/C14/C15 FAIL이 남아 있으면 **해당 시나리오를 test_{matchup}.md에서 'Y'→'X'로 되돌리고** 개별 사전 루프로 회귀 표기
```
