...
**decisive 분기에서 baseProbability는 무시된다**: `decisive: true` 이벤트가 있는 분기는 `_armyBiasedBranchSelect`로 선택되며, **누적 병력 비율 + conditionStat 능력치 차이**로 가중치가 결정된다 (0.4~0.6 캡, 즉 최대 60/40). `baseProbability`는 decisive가 없는 분기(fallback/다음 페이즈 이동)에만 유효하다.
